...
static const struct tegra_emc_table cardhu_emc_tables_samsung[] = {
	{
		0x32,       /* Rev 3.2 */
...
static const struct tegra_emc_table cardhu_emc_tables_hynix_m[] = {
	{
		0x32,       /* Rev 3.2 */
		25500,      /* SDRAM frequency */
		{
...
static const struct tegra_emc_table cardhu_emc_tables_hynix_a[] = {
	{
		0x32,       /* Rev 3.2 */
		25500,      /* SDRAM frequency */
		{
...
{
	int strap_pin_DDR_AD4 = -1;
	int strap_pin_DDR_AD5 = -1;

	struct board_info board;

	tegra_get_board_info(&board);

	gpio_request(TEGRA_GPIO_PG4, "strap_ddr_ad4");
	gpio_direction_input(TEGRA_GPIO_PG4);
	strap_pin_DDR_AD4 = gpio_get_value(TEGRA_GPIO_PG4);
	printk("DDR Strap Pin AD4: %d\n", strap_pin_DDR_AD4);
	gpio_free(TEGRA_GPIO_PG4);

	gpio_request(TEGRA_GPIO_PG5, "strap_ddr_ad5");
	gpio_direction_input(TEGRA_GPIO_PG5);
	strap_pin_DDR_AD5 = gpio_get_value(TEGRA_GPIO_PG5);
	printk("DDR Strap Pin AD5: %d\n", strap_pin_DDR_AD5);
	gpio_free(TEGRA_GPIO_PG5);


	if (strap_pin_DDR_AD5 == 1) {
		printk("Init DFS table for Hynix DDR A Die\n");
		tegra_init_emc(cardhu_emc_tables_hynix_a, ARRAY_SIZE(cardhu_emc_tables_hynix_a));
	} else {
		if (strap_pin_DDR_AD4 == 1) {
			printk("Init DFS table for Hynix DDR M Die\n");
			tegra_init_emc(cardhu_emc_tables_hynix_m, ARRAY_SIZE(cardhu_emc_tables_hynix_m));
		} else {
			printk("Init DFS table for Samsung DDR\n");
			tegra_init_emc(cardhu_emc_tables_samsung, ARRAY_SIZE(cardhu_emc_tables_samsung));
		}
	}
	return 0;
